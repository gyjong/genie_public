...
class CSVKnowledgeAgent:
    ...
    def query(self, question):
        ...
        # 변경된 부분: run() 대신 invoke() 사용
        response = self.agent.invoke(dynamic_prompt)
        
        # response가 string이 아닐 경우를 대비한 처리
        if isinstance(response, dict) and 'output' in response:
            response_text = response['output']
        else:
            response_text = str(response)

        learning_result = self.extract_knowledge(response_text)
        ...
        return response_text, learning_result
